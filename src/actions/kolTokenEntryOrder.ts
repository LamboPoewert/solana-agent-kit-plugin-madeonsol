...
export const kolTokenEntryOrderAction = {
  ...
  description:
    "Get the ranked order of KOL first-buyers for a specific Solana token. Each entry includes seconds_after_first relative to the first KOL entry. PRO+ adds percentile_pnl_7d per entry.",
  examples: [
  ...
  handler: async (agent: unknown, input: { mint: string; limit?: number }) => {
    try {
